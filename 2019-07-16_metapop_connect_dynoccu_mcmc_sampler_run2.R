...
prev_run <- readRDS("/run1_output/run1_chain01.rds")
# grab random seed
.Random.seed <- prev_run$seed
# get the final states of each parameter for new initial values
inits <- prev_run$final_state

# assign initial values based on previous run

# resistence
alpha <- inits$samples[c("alpha1","alpha2")]

# initial occupancy - only an intercept
b0.psi1 <- inits$samples["b0.psi1"]
psi1 <- plogis(b0.psi1)

# colonization
b0.gam <- inits$samples["b0.gam"] # intercept
b1.gam <- inits$samples["b1.gam"] # 1 covariate
gamma0 <- as.vector(plogis(b0.gam + b1.gam*matrix(data$sitecovs[,"size_scaled"])))
sigma <- inits$samples["sigma"]
# extinction
b0.eps <- inits$samples["b0.eps"] # intercept
b.eps <- inits$samples[c("b.eps1","b.eps2","b.eps3","b.eps4")] # 4 covariates
...
season <- c(0,inits$samples[c("season2", "season2", "season3", "season4")])
p <- plogis(a0 + season[season_vec])

# the final state of the distance matrix
D <- inits$D
...
# Some cluster set up since Distance matrix was not computed above
...
for(s in 1:iters) {
  ...
  if(s == 1){
    write.table(t(samples[s,]), "chain01_run2.csv", sep = ",",
...
saveRDS(return_list, "run2_chain01.rds")
