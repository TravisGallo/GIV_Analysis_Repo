...
# ARGO has anywhere from 16-24 cluster nodes
# Be explicit here so that we can request a 24 cluster node and it uses all 24
no_cpu <- 23
cl <- makeCluster(no_cpu)
# so we can check output to make sure it is working
cl
...
for(s in 1:iters) {
  ...
  write.table(t(samples[s,]), "mcmc_run_1/chain01.csv", sep = ",",
...
saveRDS(return_list, paste0(Sys.Date(),"_chain1_run2_list.rds"))
