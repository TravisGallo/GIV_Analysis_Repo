...
# read in final steps from previous run
prev_run <- readRDS("data.rds")
inits <- prev_run$final_state

# resistence
alpha <- inits$samples[,c("a[1]","a[2]")]

# initial occupancy - only an intercept
b0.psi1 <- inits$samples[,"b0.psi"]
psi1 <- plogis(b0.psi1)
...
b.eps <- inits$samples[,c("b.eps[1]","b.eps[2]","b.eps[3]","b.eps[4]")] # 5 covariates
