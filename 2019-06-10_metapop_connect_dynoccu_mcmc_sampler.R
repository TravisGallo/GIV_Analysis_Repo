...
# load data
load("2019-06-10_Connectivity_Workspace_ARGO.RData")

## front end cluster set up
no_cpu <- 6
cl <- makeCluster(no_cpu)
# print cluster just for a check as model is running
cl
...
nseason <- ncol(data$y_mat) # Number of sampling seasons
nsampled <- nrow(data$y_mat) # Number of sites sampled
...
# tuning parameters for Metropolis algorithm
...
# create our own function that uses the left join in data.table package
...
##########################################
##### STARTING MCMC UPDATING PROCESS #####
##########################################
...
# report the first iteration of starting values from above
if(reportit){
  cat("iter 1\n")
...
# loop that will use Metropilis algorithms to update parameter values
for(s in 1:iters) {
  ...
  #  have to include z update which is not in 'tune' hence +1
  ...
  for(subiter in 1:(length(tune)+1)){
    ...
    if(sampling_order[subiter] == 1){
      ...
      # this is needed for all parameters in early tuning
    ...
    if(sampling_order[subiter] == 2){
      ...
      # if an appropriate ll.z.cand value move on to Metropolis update
    ...
    if(sampling_order[subiter] == 12){
      ...
      for(k in 1:nseason) {
        ...
        for(i in 1:nsite) {
          ...
          if(i <= nsampled) { # only use sampled sites for log likelihood of y
  ...
  # estimated number of sites occupied each season
  zk <- colSums(z)
  
  # caculate model deviance
  deviance <- -2*ll.y.sum
  
  # percent proposals accepted 
  ...
  # fill csv with samples from each iteration and save
  if(s == 1){
    write.table(t(samples[s,]), "chain1.csv", sep = ",",
...
# state of last iteration for subsequent jobs
final_state <- list(z=z, D=D, samples=samples[s,])

# list of data we need returned
# note:remember to put zK back if we keep it
return_list <- list(samples=samples, final_state=final_state,
                    zA=zA, Ez=nz1/iters, seed=.Random.seed)

# save list
saveRDS(return_list, "run1_chain1_test.rds")
