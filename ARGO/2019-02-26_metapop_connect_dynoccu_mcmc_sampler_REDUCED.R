...
param_mon <- c("alpha[1]","alpha[2]", "alpha[3]", "sigma", 
               "b0.gam", "b.gam[1]", 
               "b0.psi1",
               ...
               "zk", "deviance")
# tuning parameters
tune <- c(0.3, 0.3, 0.3, 0.3, # alpha coefficients and sigma
...
b.gam <- rnorm(1,0,0.5) # 1 covariate
...
for(k in 2:nseason) {
  ...
  gamma[,k-1] <- 1 - Rfast::rowprods(1-G*zkt) 
...
if(reportit){
  cat("iter 1\n")
  cat("params =", round(c(alpha, b0.psi1, b0.gam, b.gam, sigma, 
...
for(s in 1:iters) {
  ...
  for(subiter in 1:(length(tune)+1)){
    ...
    if(sampling_order[subiter] == 1){
      ...
      for(k in 2:nseason) {
        zkt <- matrix(z[,k-1], nsite, nsite, byrow=TRUE)
        log_pncbn <- Matrix(log(1 - G*zkt), sparse = TRUE)
        gamma.cand[,k-1] <- parApply(log_pncbn, 1, function(x) { 1 - exp(sum(x)) })
          
        #gamma.cand[,k-1] <- 1 - exp(rowSums(log(1 - G*zkt)))
      ...
      cat("alpha1 - iteration",s,": time = ",format(Sys.time()), "\n")
    ...
    if(sampling_order[subiter] == 2){
      ...
      cat("alpha2 - iteration",s,": time = ",format(Sys.time()), "\n")
  ...
  ## Metropolis update for sigma
    if(sampling_order[subiter] == 6){
      ...
      cat("sigma - iteration",s,": time = ",format(Sys.time()), "\n")
  ...
  ## Metropolis update for b0.psi1 - part of linear predictor for initial occupancy
    if(sampling_order[subiter] == 7){
      b0.psi1.cand <- rnorm(1, b0.psi1, tune[7])
      # model
      ...
      cat("b0.psi1 - iteration",s,": time = ",format(Sys.time()), "\n")
  ...
  ## Metropolis update for b0.eps - part of the linear predictor for epsilon
    if(sampling_order[subiter] == 8){
      b0.eps.cand <- rnorm(1, b0.eps, tune[8])
  ...
  ## Metropolis update for b.eps[3] - part of the linear predictor for epsilon
    if(sampling_order[subiter] == 11){
      ...
      cat("b.eps3 - iteration",s,": time = ",format(Sys.time()), "\n")
  ...
  ## Metropolis update for b.eps[5] - part of the linear predictor for epsilon
    if(sampling_order[subiter] == 13){
      b5.eps.cand <- rnorm(1, b.eps[5], tune[13])
      epsilon.cand <- plogis(b0.eps + b.eps[1]*data$sitecovs[,"tree"] +
                                      b.eps[2]*data$sitecovs[,"total_veg"] +
                                      b.eps[3]*data$sitecovs[,"size"] +
                                      b.eps[4]*data$sitecovs[,"pop10"] +
  ...
  ## Metropolis update for a0 - part of detection probability
    if(sampling_order[subiter] == 15){
      ...
      cat("a0 - iteration",s,": time = ",format(Sys.time()), "\n")
    } # close sampler a0
    
  #Update for season 2
    if(sampling_order[subiter] == 16){
      season2.cand.vec <- season
      season2.cand.vec[2] <- rnorm(1, season[2], tune[15])
      ...
      cat("season2 - iteration",s,": time = ",format(Sys.time()), "\n")
    } # close sampler season2
  
  # Update for season 3
    if(sampling_order[subiter] == 17){
      season3.cand.vec <- season
      season3.cand.vec[3] <- rnorm(1, season[3], tune[16])
      ...
      cat("season3 - iteration",s,": time = ",format(Sys.time()), "\n")
    } # close sampler season3
  
  # Update for season 4
    if(sampling_order[subiter] == 18){
      season4.cand.vec <- season
      season4.cand.vec[4] <- rnorm(1, season[4], tune[17])
      ...
      cat("season4 - iteration",s,": time = ",format(Sys.time()), "\n")
  ...
  samples[s,] <- c(alpha, sigma, b0.gam, b.gam, b0.psi1, 
                   b0.eps, b.eps, a0, season[2:4], zk=zk, 
                   deviance=-2*ll.y.sum)
